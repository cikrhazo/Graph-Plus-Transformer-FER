...
emotion_list = ['surprise', 'fear', 'disgust', 'happiness', 'sadness', 'anger']


class eNTERFace(data.Dataset):
    '''
        1: Surprise  0
        2: Fear      1
        3: Disgust   2
        ...
        6: Anger     5
        '''
    def __init__(self, root='/media/ruizhao/programs/datasets/Face/eNTERFACE05/enterface_video/', valid=0,
                 out_size=224, window_size=49, online_ldm=False, num_frame=16, train=True):

        self.root = root
        self.root_ldm = '/media/ruizhao/programs/datasets/Face/eNTERFACE05/enterface_video_ldm/'
            ...
            for subject in dir_sub:
                ...
                for _, dir_emotions, _ in os.walk(sub_root):
                    ...
                    for emotion in dir_emotions:
                        ...
                        for _, sentences, _ in os.walk(emotion_root):
                            for sentence in sentences:
                                sentence_root = os.path.join(emotion_root, sentence)
                                self.paths.append(sentence_root)
                    break
            break
    ...
    def __getitem__(self, item):
        ...
        for _, _, files in os.walk(path):
            files.sort()
            if self.train:
                beginner = random.randint(0, 4)
                stopper = random.randint(len(files)-4, len(files))
                idxes = np.linspace(beginner, stopper - 1, self.num_frame).tolist()
                files = [files[int(i)] for i in idxes]
            else:
                idxes = np.linspace(0, len(files) - 1, self.num_frame).tolist()
                files = [files[int(i)] for i in idxes]
            for ii in range(len(files)):
                ...
                if self.online_ldm:
                    ...
                    landmark_path = img_path.split('/')[-4:]
...
if __name__ == "__main__":
    for valid in range(0, 5):
        ValidSet = eNTERFace(valid=valid, train=False, online_ldm=False)
